...
interface DvdWordmarkProps {
  className?: string;
  ariaLabel?: string;
  /**
   * Thicken the paths via SVG `feMorphology` (dilate). The DVD glyphs
   * are filled paths, not stroked, so the visual weight is fixed to
   * the path geometry — dilating expands every fill outward by
   * `dilateRadius` SVG units, faking a heavier stroke. Opt-in because
   * the footer renders the wordmark at h-[280px]+ where the paths
   * already read heavy; only the nav usage benefits.
   */
  thicken?: boolean;
  /**
   * Dilate radius in SVG userspace (viewBox is 1407×704, untransformed).
   * Defaults to 8 — enough for a perceptible bump at 48px tall, not
   * so much that the angular V shape blurs into a triangle.
   */
  dilateRadius?: number;
}

/**
 * Static inline SVG version of the DVD wordmark — uses currentColor
 * so the parent decides the fill. For the animated hero version see
 * <DvdLogo />.
 *
 * Paths are copied from public/dvd-logo.svg (viewBox 0 0 1407 704);
 * inlining them lets the SVG inherit text color in light/dark themes.
 */
export function DvdWordmark({
  className,
  ariaLabel = "dvddev",
  thicken = false,
  dilateRadius = 8,
}: DvdWordmarkProps) {
  // Unique filter id so multiple instances on the same page (nav +
  // mobile drawer header) don't collide on the SVG id namespace.
  const filterId = `dvd-thicken-${dilateRadius}`;
  return (
    <svg
